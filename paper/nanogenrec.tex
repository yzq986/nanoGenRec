...
For public reproducibility, the repository also includes a MovieLens path. The strict Colab-T4 notebook uses public ratings and metadata, Qwen3-Embedding-0.6B item representations, CPU residual KMeans SIDs, sliding-prefix NTP training, a lightweight GRPO-style exact-SID reward stage, constrained decoding, and item-recall evaluation. The earlier checked-in MovieLens 1M result uses 5,950 users, 3,532 items, 348,363 sliding-prefix training examples, and 5,950 evaluation users after filtering, but relies on lightweight hybrid title/genre and co-occurrence features and predates the public reward-alignment stage.
...
Table~\ref{tab:public-movielens} reports the checked-in MovieLens 1M Colab-T4 hybrid-feature result and simple public baselines. This result predates the strict public Qwen+RL notebook path. The nanoGenRec run uses hybrid public item features, residual KMeans SIDs with a 64$\times$64$\times$64 token space, a dense 3-layer NTP model with embedding dimension 128, and SID-constrained beam search with beam size 1000.
...
nanoGenRec public & 5,950 & 3,532 & 348,363 & 10.5\% & 40.4\% & 72.5\% & 85.2\% \\
\bottomrule
\end{tabular}
\end{table}

The nanoGenRec public run also obtains R@1=1.9\%, R@5=6.2\%, R@50=29.0\%, and target-SID found rate of 89.9\%. It beats global popularity at all reported cutoffs, but a simple item co-occurrence KNN remains stronger on this small and dense collaborative-filtering dataset. This reinforces the intended claim boundary: the MovieLens path is a public end-to-end reproducibility check for the framework, not a public leaderboard claim.
...
This report has several limitations. The production behavior data is not released, so external users cannot exactly reproduce the reported production-scale numbers. The repository includes MovieLens CPU and Colab-T4 public paths to verify the released framework loop, but these are reproducibility checks rather than competitive public leaderboard claims. On the checked-in MovieLens 1M hybrid-feature run, a simple item co-occurrence KNN baseline is stronger than nanoGenRec, which shows that the public path should not be interpreted as a tuned public recommender. The strict public Qwen+RL path is provided as an executable notebook and should be registered as a new result after a full T4 run. The framework currently focuses on Semantic-ID autoregressive recommendation rather than all forms of generative retrieval. Online serving, latency, and production integration are not the main focus of the artifact. Some post-training methods are preliminary and should be interpreted as framework-validated engineering evidence rather than final algorithmic conclusions. The most important next step is broader public benchmarking across redistributable datasets and stronger public baselines, so external users can verify not only the code structure but also cross-dataset empirical trends.
