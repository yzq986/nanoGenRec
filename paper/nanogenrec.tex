...
\begin{abstract}
Generative recommendation reframes retrieval as autoregressive generation over item Semantic IDs, but making this paradigm work in practice requires more than a model definition. Item tokenization, behavior-sequence construction, next-token training, reward alignment, constrained decoding, full-recall evaluation, experiment governance, and failure diagnosis must form a single executable loop. We present \textsc{nanoGenRec}, an open-source agentic research framework for landing Semantic-ID generative recommendation. The framework couples a runnable GR stack with an agent-facing research protocol: inbox/outbox communication, paper notes, YAML experiment configuration, duplicate-run detection, runtime estimation, queue-based long jobs, full-recall evaluation, decision records, and durable experiment logs. The project is distilled from an anonymized production-scale experiment lineage: 51 recorded experiments, behavior sweeps up to 7.85M users and 299.0M raw interactions, roughly 445M effective SID tokens after truncation, million-item SID caches, and private runs executed with up to four NVIDIA H200 GPUs. Within this lineage, \textsc{nanoGenRec} records model-scaling behavior across 1.7M--101.1M active parameters, non-monotonic behavior-window scaling under temporal drift, tokenizer sweeps connecting embedding scale and codebook balance to downstream recall, SID-constrained full-recall retrieval runs, and post-training cases where rollout distribution mismatch causes alignment collapse. The released artifact removes private data and deployment details while preserving the operating system around the research: configs, logs, validation checks, public notebooks, and reproducible code paths. A checked-in MovieLens 1M Colab-T4 path verifies that the public code can execute the same Qwen embedding $\rightarrow$ SID construction $\rightarrow$ NTP training $\rightarrow$ reward-alignment $\rightarrow$ full-evaluation loop without private data. We position \textsc{nanoGenRec} as a landing record and reproducible research workspace for researchers and agents turning Generative Recommendation from paper-level recipes into working systems.
\end{abstract}
...
For public reproducibility, the repository also includes a MovieLens path. The strict Colab-T4 run uses public ratings and metadata, Qwen3-Embedding-0.6B item representations, residual KMeans SIDs, 348,363 sliding-prefix NTP training examples, 348,363 alignment examples, a lightweight GRPO-style exact-SID reward stage, constrained decoding, and item-recall evaluation over 1,000 held-out users. An earlier checked-in MovieLens 1M result uses the same users and evaluation split but relies on lightweight hybrid title/genre and co-occurrence features and predates the public reward-alignment stage.

\subsection{Compute}

The production-scale empirical experiments were run with up to four NVIDIA H200 GPUs. This hardware note is included only to make the compute scale of the experiment lineage interpretable; the released code does not depend on that environment. The public MovieLens proof run was executed on a single free Colab T4 GPU.
