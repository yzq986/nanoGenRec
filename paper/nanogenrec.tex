...
\small
\textbf{Production path:} Qwen3 item embeddings $\rightarrow$ RKMeans/FSQ SIDs $\rightarrow$ NTP shards $\rightarrow$ Transformer+MoE $\rightarrow$ constrained full recall $\rightarrow$ EXP logs \\
\vspace{0.4em}
\textbf{Public path:} MovieLens metadata/co-occurrence hashes $\rightarrow$ 64$\times$64$\times$64 SIDs $\rightarrow$ sliding-prefix shards $\rightarrow$ dense NTP $\rightarrow$ constrained full recall $\rightarrow$ checked-in public metrics \\
\vspace{0.4em}
...
Table~\ref{tab:public-movielens} reports the current MovieLens 1M Colab-T4 result and simple public baselines. The nanoGenRec run uses hybrid public item features, residual KMeans SIDs with a 64$\times$64$\times$64 token space, a dense 3-layer NTP model with embedding dimension 128, and SID-constrained beam search with beam size 1000.
