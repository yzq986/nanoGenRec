...
\begin{itemize}
  ...
  \item a CPU-friendly MovieLens public smoke path;
...
This report has several limitations. The production behavior data is not released, so external users cannot exactly reproduce the reported production-scale numbers. The repository includes a CPU-friendly MovieLens smoke path to verify the public framework loop, but it is not intended as a competitive public benchmark. The framework currently focuses on Semantic-ID autoregressive recommendation rather than all forms of generative retrieval. Online serving, latency, and production integration are not the main focus of the artifact. Some post-training methods are preliminary and should be interpreted as framework-validated engineering evidence rather than final algorithmic conclusions. The most important next step is a stronger public benchmark section on redistributable datasets, so external users can verify not only the code structure but also the reported empirical trends.
