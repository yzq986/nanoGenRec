...
| 优先级 | ID | 实验 | 原因 |
|--------|-----|------|------|
| P0 | IDEA-gr4ad-0 | MGMR 不等大码本 | 零成本改进，直接提升 collision/utilization；与现有 RKMeans 代码完全兼容 |
| P1 | IDEA-gr4ad-1 | LazyAR 解码器 | 与 ARCHITECTURE.md Lazy Decoder-Only 方向一致；扩展 token 数或 beam 后必需 |
| P1 | IDEA-gr4ad-2 | Value-Aware 训练 | 丰富训练信号，与 IDEA-sid-1 互补；eCPM token 实现简单 |
| P1 | IDEA-gr4ad-4 | Dynamic Beam Search | 生产 beam=512 时必需；可与 IDEA-gr4ad-0 配套 |
| P2 | IDEA-gr4ad-3 | RSPO 排序优化 | 收益最大但前置依赖最重；建议系统成熟后再做 |
