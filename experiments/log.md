...
Easy DPO 显著提升短距离检索（R@10 +26%）和逐层判别力（depth_acc +37~61%），代价是 PPL 略升和长尾 R@500 下降。符合预期。
...
Config 1, 2 及 λ 消融待完成。

### Analysis

1. **PPL vs Recall tradeoff**: DPO 优化排序（chosen > rejected），不优化绝对概率。PPL 上升但 Recall 改善 — 模型学会"排对"而非"概率高"。
...
2. 如果渐进模型更优，跑 λ 消融确定最优权重
3. EXP-018: RF-DPO（引入真实用户反馈，Align³GR Phase 2）
