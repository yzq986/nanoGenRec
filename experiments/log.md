...
| exp038-hard-lam03 | **9.4%** | 23.7% | 33.0% | **57.2%** | **27.3** | 358s |

**⚠️ 异常：DPO 未生效** — `n_dpo_pairs=0`, `avg_dpo_loss=0.0`。训练实际上是纯 NTP fine-tune（以 exp037-medium 为起点），而非 RF-DPO。结果等价于在 sp-dpo checkpoint 上再训一轮 NTP，反而比 exp037-medium 退化（R@500: 62.1%→57.2%，PPL: 23.0→27.3）。

### Analysis

DPO 未生效的根本原因：`sp-dpo-train` 命令读取 preference pairs 时 `n_dpo_pairs=0`，说明 exp018 hard pairs 格式或路径与当前训练脚本接口不匹配（可能是 pair 文件列举逻辑、difficulty=hard filter、或 pair 文件本身为空）。需检查 `experiments/rf_dpo_data/exp018/hard/` 目录内容及 `trainer.py` 的 pair 加载逻辑。

结果退化（比 exp037-medium 差）是预期的——在已经做过 SP-DPO 的 checkpoint 上再跑一轮纯 NTP 会略微丢失对齐信号。

### Next Steps
- 诊断 exp038 DPO 未生效原因（检查 exp018/hard/ pair 文件）
- EXP-039 ECPO 可先用 exp038-hard-lam03 作为 checkpoint 继续链路（ECPO 有独立的 reward 机制，不依赖 DPO pairs）
- 或修复 DPO 加载后重跑 EXP-038（需确认 exp018 hard pair 文件存在且格式正确）
