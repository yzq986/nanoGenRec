...
# EXP-039: ECPO on exp038-hard-lam03 (Features 路线第四步 — RL 链路终点)
# Date: 2026-04-28
#
# RL 对齐链路:
#   exp036-full-features (SFT)
#   → EXP-037 SP-DPO → exp037-medium
#   → EXP-038 RF-DPO → exp038-hard-lam03   (ref for this exp)
#   → [本实验: ECPO δ=0.1]
#
# 对标: EXP-029 ECPO on-policy (非 features 版本)
# 验证 features + 完整 RL 链路能否超越 exp020 baseline (R@500=66.2%)
...
SFT_CKPT="experiments/ntp_checkpoints/exp038-hard-lam03"   # RF-DPO output from EXP-038
...
echo "=========================================="
echo "EXP-039: ECPO on exp038-hard-lam03"
echo "=========================================="
echo "  GPUs:           ${N_GPUS}"
...
if [ ! -f "${SFT_CKPT}/train_meta.json" ]; then
    echo "ERROR: exp038-hard-lam03 checkpoint not found at ${SFT_CKPT}"
...
if [ "${SKIP_SMOKE}" == false ]; then
    SMOKE_OUT="${CKPT_DIR}/exp039-smoke"
    echo ""
...
if [ -f "${OUTPUT}/probe.pt" ] && [ "${FORCE}" != true ]; then
    ...
    echo ">>> Training: ${NAME}"
    echo "    ref=exp038-hard-lam03, ECPO δ=0.1, G=512, BehaviorReward+FormatReward, on-policy beam"
    echo ""
    torchrun --nproc_per_node="${N_GPUS}" run.py grpo-train \
        --sft_checkpoint "${SFT_CKPT}" \
        --preprocessed_dir "${NTP_DATA}" \
...
git add experiments/
git commit -m "EXP-039 results: ECPO features RL chain complete" || echo "Nothing to commit"
./push.sh

echo ""
echo "EXP-039 complete! Features RL alignment chain finished."
