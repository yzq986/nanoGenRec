...
模型：632M total params，103.9M active params（16E top-2）。

### Analysis

1. **R@500 64.1%，低于预期（72-74%）**：L-tier + 全部优化的结果仅比 S-tier + TO-RoPE（63.9%）提升 0.2pp，
   远低于 M-tier bare（70.2%）。根本原因：**本实验是 1 epoch，M-tier（EXP-043）也是 1 epoch**，
   但 L-tier（101M active）相比 M-tier（71.6M active）参数多 40%，1 epoch 下 L-tier 更严重欠训练。
   Chinchilla 最优 tok/param 比：L-tier 约 1.3 tok/param（132M tokens / 103M active），远低于推荐的 20x。

2. **PPL=20.7 比 M-tier bare（18.54）高**：同样是欠训练的体现，更大的模型在相同数据量下拟合更差。

3. **TO-RoPE + gate_attn 在 L-tier 上收益不明显**（vs M-tier bare 对比）：
   多轮优化（TO-RoPE+gate）带来的收益被 scale 惩罚抵消。需要更多训练 tokens 才能体现大模型优势。

4. **RL 链路起点仍然可用**：R@500=64.1% > S-tier SFT（53-61%），可以作为 SP-DPO 起点。
   但若目标是超越 exp039b-ecpo（65.7%），需要 SFT 先达到 ≥62%，当前 64.1% 满足条件。

5. **结论**：L-tier 在单 epoch 下不如 M-tier。如需发挥 L-tier 优势，需要 ≥3 epoch 或更多数据。
   RL 链路以 exp047 为起点继续推进，观察 RL 后的最终效果。

### Next Steps

- RL 链路：exp047 (R@500=64.1%) → SP-DPO → RF-DPO (3ep) → ECPO
...
EXP-044B 验证了 TO-RoPE（timestamps 接通后）在 S-tier 上有效（R@500=63.6%，+2.4pp）。
EXP-047 在 L-tier 上 TO-RoPE 收益被欠训练掩盖（1.3 tok/param）。

本实验回到 M-tier（参数量已充分验证），测试：
- 2-dim RoPE：order:0.5 + time:0.5（EXP-044B S-tier 最优配置）
- 3-dim RoPE：order:0.4 + time:0.5 + layer:0.1（新增 layer_id 维度）
...
- **Data**: exp044b-0.6b-14d（timestamps 已接通）；SID: exp026-0.6b-14d
- **LR**: 1e-3
...
1. **TO-RoPE 在 M-tier 上无收益**：2-dim 与 baseline 持平（-0.1pp），3-dim 小幅下跌（-0.7pp）。
   与 S-tier 结论不同（EXP-044B S-tier +2.4pp）——M-tier 更强的模型容量可能已通过绝对位置编码充分捕捉时序信息，RoPE 的边际贡献消失。

2. **3-dim < 2-dim**：layer:0.1 分量占用了原本给 order 的头维度（0.5→0.4），R@10 更高（15.5% vs 15.1%）但 R@500 更低，说明 layer_id 维度提升了近邻精度但损害了长尾召回。

3. **PPL 均有改善**（17.6 vs 18.54），说明 RoPE 在语言建模层面确实有收益，但未转化为 R@500。

4. **结论**：TO-RoPE 对 M-tier 帮助有限，不建议在 M-tier + 1 epoch 配置下使用。后续实验保持 M-tier bare（70.2%）或加 gate_attn（exp046，61.6%，S-tier 版本）作为基准。

### Next Steps

- M-tier bare (exp043, R@500=70.2%) 仍为当前 NTP 最优 checkpoint
- L-tier 需要 ≥3 epoch 才能体现优势，暂不推进
- 下一步优先：RL 链路（SP-DPO → RF-DPO → ECPO）
...
### Design
- **0.6b sweep**: h ∈ {32, 64(reuse→重跑), 128, 256}
...
**⚠️ 重要 bug（2026-04-30 发现）**：EXP-045 所有新跑的 preprocess-sid 均使用 `num_clusters=1024`（默认值），而 exp026 使用的是 `num_clusters=4096`。exp-045.sh 有 `--fsq_levels 12d_4096` 但漏传了 `--num_clusters 4096`，导致 KMeans 聚类欠约束。**所有 exp045-* 目录的数据不可与 exp026 直接比较，结论需用 `--num_clusters 4096` 重跑验证。** exp026 系列数据仍然可信。
...
| ~~0.6b~~ | ~~1024~~ | ~~64~~ | ~~0.063~~ | ~~0.49%~~ | ~~—~~ | ~~1,096,364~~ | ~~4096~~ | ~~数据集不同，作废~~ |
...
Gini_d2 = L1+L2 prefix 分布 Gini 系数（FORGE proxy，越低越均匀，与 NTP L2 预测难度正相关）。

**⚠️ 修正 #1**：0.6b h=64 旧复用结果（0.49%）使用了不同数据集（n_items 差 14k），已重跑；但重跑本身也有 num_clusters bug。

**⚠️ 修正 #2（num_clusters bug）**：exp045 系列 Gini_d2 = 0.54–0.57，而 exp026 = 0.33–0.35，差距完全由 num_clusters=1024 vs 4096 引起。需用 `--num_clusters 4096` 重跑才能得出可信的 h vs quality 曲线。

### Analysis

1. **num_clusters bug 是主因**：exp045 Gini_d2 (0.54–0.57) vs exp026 (0.33–0.35)，差距显著。Gini_d2 衡量 L2 前缀分配均匀度，直接影响 NTP 层间预测难度。bug 未修前所有 h-sweep 结论均不可信。

2. **Gini_d2 > Gini_d3（collision）更有信息量**：在 FSQ codebook 容量（4096³）远大于 item 数（1.1M）的情况下，Gini_d3 ≈ collision rate 的另一种表达。Gini_d2 捕捉 KMeans 层间负载均衡，与 NTP 分层预测质量更直接相关。

3. **4b collision 对 h 不敏感**（即使修 bug 后该结论可能仍成立）：根本原因是 12d_4096 codebook 对 1.1M items 有理论 collision 下界。要降低 4b collision，需增加 FSQ levels。

4. **8b embedding cache 数据问题**：item_id 与 behavior data 对齐率仅 2.3%，需重建。

### Next Steps
- **必做**：用 `--num_clusters 4096` 重跑 EXP-045 sweep（0.6b h=32/64/128/256，4b h=128/512/1024）
- exp026-0.6b-14d（h=64，num_clusters=4096，Gini_d2=0.33，CR=0.49%）仍为当前最可信的 0.6b SID
- exp026-4b-14d（h=64，num_clusters=4096，Gini_d2=0.35，CR=2.76%，R@500=70.4%）仍为当前最好的 4b SID
...
⚠️ **2026-04-30 re-eval**：修复 teacher-forced eval timestamps bug（之前全零），PPL 已更正为真实值。
...
1. **PPL 修复后结论有变化**：044C 系列 PPL 22-23，与 baseline（26.5）相近甚至更低，说明 item-level position fix（pos//L）确实改善了 teacher-forced eval 的质量。044B PPL 47.5 偏高，因为 044B 用 token-level position，logit 分布熵更高。

2. **R@500 结论不变**：044C-B（ts=0.5）最佳 63.9%，3-dim RoPE 无收益，2-dim ts=0.5 为推荐配置。

3. **044B re-eval R@500 下降**（63.6% → 54.3%）：重新 eval 用 n_recall=1000，且 timestamps 现在真实传入。说明旧 eval 的 R@500=63.6% 也有部分虚高（timestamps 全零等价于关闭时间维度，退化为纯 order RoPE，反而更稳定？需进一步确认）。
...
1. **TO-RoPE 有效（044B < 044C）**：修复 teacher-forced eval timestamps bug 后，044B R@500 为 53.8-56.2%（低于 baseline 61.2%）。044C 用 item-level position（pos//L）达到 63.5-63.9%，确认 item-level position 是关键。

2. **PPL 修复后 044B PPL=40-48（合理）**：之前 PPL=467-480 是 bug 结果（timestamps=0）。修复后 PPL 40-48，比 baseline（26.5）高约 50%，L2 PPL=18-28（044C L2 PPL≈3.5 因 item-level 使 L2 预测极易）。

3. **time_split=0.25 best R@500（54.3%）但 PPL 最高（47.5）**：更多位置维度留给 order-RoPE 时，R@500 略高但 L2 PPL 更高（27.7 vs 18.0），说明 ts=0.25 对时间信息编码弱，模型需更多 order 信号。
