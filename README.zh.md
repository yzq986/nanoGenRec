...
flowchart LR
    ...
    M16 ==>|"SP-DPO self-play"| M17
    M17 ==>|"Joint NTP+RF-DPO\nref=SP-DPO ckpt"| M20
    M20 ==>|"ECPO on-policy beam"| M029

    %% ═══════════════════════════════════════════════
    %% 支线 1：RF-DPO 失败尝试（pure DPO 灾难遗忘）
    %% ═══════════════════════════════════════════════
    E18["❌ EXP-018\nPure RF-DPO\nPPL=50K+"]
    E19["EXP-019\nλ sweep"]

    M17 -.->|"去掉 NTP loss"| E18
    M17 -->|"Joint NTP+RF-DPO"| E19
    E19 -->|"λ=0.03 甜点"| M20
    ...
    M029 -->|"更精细 advantage"| E030
    E030 -->|"exp020 起点"| M031B

    %% ═══════════════════════════════════════════════
    %% 支线 3：Features 探索 → 当前路线
    %% ═══════════════════════════════════════════════
    M23["exp023\nseg_emb only\n61.2%"]
    E022["❌ EXP-022\nInfoNCE → 全失败"]
    E024["❌ EXP-024\n推理侧 feature 泄漏"]
    ...
    M23 -.->|"InfoNCE 对比学习"| E022
    M23 -->|"加 time_gap+action"| E024
    E024 -->|"beam 推理补传 features"| M25
    M20 -->|"干净对照\n验证 features 效果"| M036B
    M036B ==>|"SP-DPO 重新生成 pairs"| E037
    E037 ==>|"Joint NTP+RF-DPO\nref=SP-DPO ckpt"| F038
    F038 ==>|"ECPO on-policy"| F039

    %% ═══════════════════════════════════════════════
    %% 支线 4：Features RL 失败尝试
    %% ═══════════════════════════════════════════════
    E031A["❌ EXP-031A\nexp025 起点\nclip=96%"]
    E033["❌ EXP-033\nfeatures bug 证伪"]
    E034["⚠ EXP-034\nref=exp025\nclip 仍 95%"]
    ...
    M025_rl["exp025"]
    M25 -.-> M025_rl
    E030 -.->|"features SFT 起点"| E031A
    M025_rl -.-> E031A
    E031A -.->|"怀疑 features bug"| E033
    E033 -.->|"ref 换成 policy 起点"| E034
    E034 -.->|"sampling ρ≈1"| E035
    ...
    %% 样式
    %% ═══════════════════════════════════════════════
    classDef main fill:#1d3461,color:#fff,stroke:#1d3461,stroke-width:2px
    classDef sota fill:#1d3461,color:#ffd700,stroke:#ffd700,stroke-width:3px
    classDef failed fill:#6c757d,color:#fff,stroke:#495057
    classDef running fill:#e76f51,color:#fff,stroke:#b5451c,stroke-width:2px
    classDef planned fill:#457b9d,color:#fff,stroke:#1d3557
    classDef warn fill:#f4a261,color:#000,stroke:#e76f51
    classDef side fill:#2d6a4f,color:#fff,stroke:#1b4332

    class M16,M17,M20,M029 sota
    class E030,M031B,M036B,E037 main
    class M23,M25 side
